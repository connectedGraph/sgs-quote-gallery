window.sakuraControl = {
    ...
    toggle: function() { 
        toggleSakuraEnabled(); 
        console.log('樱花效果已' + (sakuraEnabled ? '开启' : '关闭'));
    },
    fadeOut: function(callback) { 
        fadeOutSakura(callback); 
    },
    fadeIn: function() { 
        fadeInSakura(); 
    },
    isEnabled: function() { 
        return sakuraEnabled; 
    }
};
// 樱花状态切换函数（调用你已有的 sakuraControl）
function toggleSakura() {
  // 调用你的切换方法
  sakuraControl.toggle();
  
  // 获取按钮元素，更新文字和图标
  const btn = document.getElementById('sakuraToggle');
  // 这里根据 sakuraControl 的实际状态判断（你可以根据自己的逻辑调整，比如用一个全局变量记录状态）
  if (btn.innerText.includes('开启')) {
    btn.innerHTML = '<span>❌</span> 关闭樱花';
  } else {
    btn.innerHTML = '<span>🌸</span> 开启樱花';
  }
  
  // 如果你能直接获取樱花的状态，也可以这样写：
  // if (sakuraControl.isEnabled) { // 假设你的 sakuraControl 有 isEnabled 状态属性
  //   btn.innerHTML = '<span>❌</span> 关闭樱花';
  // } else {
  //   btn.innerHTML = '<span>🌸</span> 开启樱花';
  // }
...
var stop, staticx;
var img = new Image();
// 使用有效的樱花图片（这里使用一个示例URL，你可以替换为自己的图片）
...
// 添加全局控制变量
var sakuraEnabled = true; // 默认开启樱花动画
var sakuraFadeOutTimer = null;
var sakuraFadeInTimer = null;
var sakuraOpacity = 1; // 初始不透明度
var isFadingOut = false; // 是否正在淡出
var isFadingIn = false; // 是否正在淡入
var canvasSakura = null; // 保存canvas引用
...
function Sakura(x, y, s, r, fn) {
    ...
    this.r = r;
    this.fn = fn;
    this.opacity = 1; // 每朵樱花单独的不透明度
    this.isActive = true; // 樱花是否活跃
}

Sakura.prototype.draw = function(cxt) {
    if (!this.isActive) return; // 不活跃的樱花不绘制
    ...
    // 应用全局和个体的不透明度
    cxt.globalAlpha = sakuraOpacity * this.opacity;
    cxt.drawImage(img, 0, 0, 40 * this.s, 40 * this.s);
    cxt.restore();
};

Sakura.prototype.update = function() {
    if (!this.isActive) return; // 不活跃的樱花不更新
    ...
    // 淡出效果
    if (isFadingOut && this.opacity > 0) {
        this.opacity -= 0.02; // 每帧减少不透明度
        if (this.opacity <= 0) {
            this.isActive = false; // 完全透明后设为不活跃
        }
    }
    
    // 淡入效果
    if (isFadingIn && this.opacity < 1) {
        this.opacity += 0.02; // 每帧增加不透明度
...
SakuraList.prototype.resetOpacity = function() {
    for(var i = 0, len = this.list.length; i < len; i++) {
...
function startSakura() {
    ...
    if (existingCanvas) {
        canvasSakura = existingCanvas;
        return; // 如果已经存在，则不再创建
    }
    
    canvasSakura = document.createElement('canvas');
    var cxt;
    staticx = true;
    canvasSakura.height = window.innerHeight;
    canvasSakura.width = window.innerWidth;
    canvasSakura.setAttribute('style', 'position: fixed;left: 0;top: 0;pointer-events: none;z-index: 9999;');
    canvasSakura.setAttribute('id', 'canvas_sakura');
    document.getElementsByTagName('body')[0].appendChild(canvasSakura);
    
    cxt = canvasSakura.getContext('2d');
    ...
    function animate() {
        if (!sakuraEnabled || !canvasSakura) return;
        
        cxt.clearRect(0, 0, canvasSakura.width, canvasSakura.height);
        sakuraList.update();
        sakuraList.draw(cxt);
...
// 淡出效果
function fadeOutSakura(callback) {
    ...
    // 清除之前的淡入淡出定时器
    if (sakuraFadeOutTimer) clearInterval(sakuraFadeOutTimer);
    if (sakuraFadeInTimer) clearInterval(sakuraFadeInTimer);
    
    sakuraFadeOutTimer = setInterval(function() {
        // 逐渐减少全局不透明度
        sakuraOpacity -= 0.02;
        
        // 当不透明度为0时，完全停止动画并清除canvas
        if (sakuraOpacity <= 0) {
            sakuraOpacity = 0;
            clearInterval(sakuraFadeOutTimer);
            sakuraFadeOutTimer = null;
            isFadingOut = false;
    ...
    }, 30); // 每30ms执行一次，大约1秒完成淡出
}

// 淡入效果
function fadeInSakura() {
    ...
    // 重置樱花的不透明度
    if (canvasSakura) {
        var cxt = canvasSakura.getContext('2d');
        var sakuraList = new SakuraList();
        // 这里需要获取现有的樱花列表，实际项目中可能需要保存sakuraList引用
    }
    ...
    sakuraFadeInTimer = setInterval(function() {
        // 逐渐增加全局不透明度
        sakuraOpacity += 0.02;
        ...
        if (sakuraOpacity >= 1) {
            sakuraOpacity = 1;
            clearInterval(sakuraFadeInTimer);
            sakuraFadeInTimer = null;
            isFadingIn = false;
        }
    }, 30); // 每30ms执行一次，大约1秒完成淡入
}

// 页面加载完成后1秒逐渐隐藏樱花
function autoFadeOutSakura() {
    if (sakuraEnabled) {
        setTimeout(function() {
            fadeOutSakura(function() {
                // 淡出完成后禁用樱花
                sakuraEnabled = false;
                // 移除canvas元素
                if(canvasSakura && canvasSakura.parentNode) {
...
// 新功能：自定义开启/关闭樱花动画（带淡入淡出效果）
function setSakuraEnabled(enabled) {
    if (enabled === sakuraEnabled) return; // 状态相同则不执行
    
    if (enabled) {
        // 启用樱花，先淡入
        sakuraEnabled = true;
        startSakura(); // 先创建canvas
        // 从透明开始淡入
        sakuraOpacity = 0;
        fadeInSakura();
    } else {
...
img.onerror = function() {
    console.error('樱花图片加载失败，请检查图片URL或Base64数据');
    // 如果图片加载失败，可以在这里提供一个备用方案
    // 例如：img.src = 'data:image/png;base64,...'; // 使用Base64编码的备用图片
};
...
if(document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        // 等待页面所有内容加载完成
        window.addEventListener('load', autoFadeOutSakura);
    });
} else {
    window.addEventListener('load', autoFadeOutSakura);
}